...
var AiMediaService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.AiMediaService = void 0;
...
let AiMediaService = AiMediaService_1 = class AiMediaService {
    base;
    mediaService;
    logger = new common_1.Logger(AiMediaService_1.name);
    constructor(base, mediaService) {
        this.base = base;
        this.mediaService = mediaService;
    }
    async generateImage(prompt, aspectRatio = '1:1', userId) {
    ...
    async generateSpeech(body, userId) {
        if (!this.base.currentFptKey)
            throw new common_1.BadRequestException('FPT AI API Key chưa được cấu hình.');
        ...
        if (!cleanedText)
            return { url: null, success: false, message: 'Nội dung trống' };
        const tryCall = async (tier, retryCount = 0) => {
            try {
                if (tier === 'v5') {
                    const v5Res = await (0, axios_1.default)({
                        method: 'post',
                    ...
                    const v3Res = await axios_1.default.post('https://api.fpt.ai/hmi/tts/v3', params.toString(), {
                        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                        timeout: 30000
                    });
                    if (v3Res.data?.async)
                        return v3Res.data.async;
                }
                else {
        ...
        try {
            ...
            throw new common_1.InternalServerErrorException('Lỗi khi gọi API FPT AI: ' + error.message);
        }
    }
    async generateImageMockup(prompt, productImage, logoImage, modelImage, aspectRatio, userId) {
        if (!this.base.genAI)
        ...
        try {
            ...
            try {
                ...
                if (logoBase64)
                    promptParts.push({ inlineData: { data: logoBase64, mimeType: "image/jpeg" } });
                if (modelBase64)
            ...
            for (const modelName of imageModels) {
                for (let attempt = 1; attempt <= 2; attempt++) {
                    try {
                        ...
                        const finalParts = [{ text: enhancedPrompt }];
                        const prodBase64 = productImage ? await this.base.resolveBase64Image(productImage) : null;
                        const logoBase64 = logoImage ? await this.base.resolveBase64Image(logoImage) : null;
                        const modelBase64 = modelImage ? await this.base.resolveBase64Image(modelImage) : null;
                        if (prodBase64)
                        ...
                        if (logoBase64)
                            finalParts.push({ inlineData: { data: logoBase64, mimeType: "image/jpeg" } });
                        if (modelBase64)
            ...
            throw new common_1.InternalServerErrorException('Không thể tạo ảnh. Vui lòng thử lại sau!');
        }
    }
    async generateVisualClone(modelImage, templatePrompt, userId, templateImage, count = 1, fidelity = 90, creativity = 50) {
        ...
        try {
            ...
            for (const modelName of googleModels) {
                ...
                finalUrls = results.filter((url) => !!url);
            }
            if (finalUrls.length === 0)
    ...
    async callSingleGeminiImageGen(modelName, parts, userId, aspectRatio) {
        if (!this.base.genAI)
            return null;
        try {
            ...
            const imagePart = result.response.candidates?.[0]?.content?.parts?.find((p) => p.inlineData);
            if (imagePart?.inlineData)
                return await this.uploadBase64ToStorage(imagePart.inlineData.data, imagePart.inlineData.mimeType || 'image/jpeg', userId, aspectRatio || '1:1');
    ...
    async generateKocProductImage(kocId, productImage, prompt, userId, modelOverride, bgImage) {
        if (!this.base.genAI)
            throw new common_1.BadRequestException('Gemini API Key chưa được cấu hình.');
        const db = admin.firestore();
        ...
        try {
            ...
            if (!modelBase64 || !productBase64)
                throw new common_1.BadRequestException('Không thể xử lý hình ảnh đầu vào.');
            const parts = [];
            if (bgBase64)
    ...
    async downloadUniversalVideo(urlInput, userId) {
        ...
        if (platform === 'Facebook') {
            try {
                ...
                const fbRes = await axios_1.default.post('https://getmyfb.com/process', params, {
        ...
        try {
            const res = await axios_1.default.post('https://api.cobalt.tools/api/json', { url, videoQuality: '1080' }, { headers: { 'Accept': 'application/json' }, timeout: 15000 });
            if (res.data?.url)
                return {
        ...
        throw new common_1.BadRequestException(`Không thể tải video từ ${platform}. Vui lòng thử lại sau.`);
    }
    detectPlatform(url) {
    ...
    async downloadTikTokVideo(url, userId, skipDeduction = false) {
        ...
        if (!skipDeduction)
            await this.base.deductCredits(userId, this.base.CREDIT_COSTS.VIDEO_DOWNLOADER, 'Tải video TikTok', 'VIDEO_DOWNLOADER');
        try {
            let cleanUrl = url.trim().split('?')[0];
            if (cleanUrl.includes('v.douyin.com')) {
                const res = await axios_1.default.get(cleanUrl, { maxRedirects: 5, timeout: 10000 });
            ...
            throw new common_1.BadRequestException('Không thể tải video TikTok/Douyin.');
        }
    }
    async uploadBase64ToStorage(base64, mimeType, userId, aspectRatio) {
        try {
    ...
    async getAiKocs(userId) {
        ...
        try {
            ...
            throw new common_1.InternalServerErrorException(error.message || 'Lỗi hệ thống khi tải KOC.');
        }
    }
    ...
    async proxyDownload(url, filename, res) {
        try {
            const response = await (0, axios_1.default)({ method: 'get', url: url, responseType: 'stream' });
...
exports.AiMediaService = AiMediaService = AiMediaService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [ai_base_service_1.AiBaseService,
        media_service_1.MediaService])
], AiMediaService);
//# sourceMappingURL=ai-media.service.js.map