...
export class AiTikTokService {
    ...
    async generateTikTokAIAnalysis(user: any, stats: any, videos: any[], retryCount = 0): Promise<any> {
        if (!this.base.model) return this.getDefaultAIAnalysis();
    ...
    async generateTikTokVideoScript(uniqueId: string, niche: string, userId: string): Promise<any> {
        ...
        try {
            ...
            throw new BadRequestException("Không thể tạo kịch bản lúc này.");
        }
    }
    ...
    async getTikTokTrending(region = 'VN', count = 50, refresh = false, category?: string, userId?: string): Promise<any> {
        try {
            ...
            // Mapping về cấu trúc chuẩn để React không bị lỗi play_count undefined
            const trendingVideos = rawVideos.map((v: any) => ({
                id: v.video_id || v.id,
            ...
            throw new BadRequestException('Lỗi hệ thống TikTok hoặc tài khoản: ' + error.message);
        }
    }
    ...
    async downloadTikTokVideo(url: string, userId: string): Promise<any> {
        ...
        try {
            ...
            if (data?.code === 0 && data.data) {
                const v = data.data;
                // Mapping về cấu trúc mà TikTokDownloadPage.tsx mong đợi
                return {
                    id: v.id || v.video_id,
                    title: v.title || '',
                    cover: v.cover || v.origin_cover || '',
        ...
        throw new BadRequestException('Không thể tải video từ link này. Vui lòng kiểm tra lại URL.');
    }
